# 손실 함수의 성능 비교: 평균 제곱 오차와 교차 엔트로피
import numpy as np
...
# 두 모델의 정확률 비교
result_mse = dmlp_mse.evaluate(x_test, y_test, verbose=0)
print('평균제곱오차의 정확률은 ', result_mse[1] * 100)
result_ce = dmlp_ce.evaluate(x_test, y_test, verbose=0)
print('교차 엔트로피의 정확률은 ', result_ce[1] * 100)
...
plt.title('Model accuracy comparsion between MSE and cross entropy')
plt.xlabel('Epoch')
plt.ylabel('Accuracy')
